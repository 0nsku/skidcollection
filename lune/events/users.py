...
class Users(commands.Cog): 
  ...
  @commands.Cog.listener()
  async def on_member_unban(self, guild: discord.Guild, user: discord.User):
         check = await self.bot.db.fetchrow("SELECT * FROM developerhardban WHERE guild_id = {} AND banned = {}".format(guild.id, user.id))
         if check is not None: 
            await guild.ban(user, reason=f"user has been developer hardbanned by {await self.bot.fetch_user(check['author'])} (ask a developer to unban)")
   
  @commands.Cog.listener('on_member_remove')
  async def booster_left(self, member: discord.Member): 
      if member.guild.id == 1170234585414643742:
  ...
  async def booster_unboosted(self, before: discord.Member, after: discord.Member): 
    if before.guild.id == 1170234585414643742:
