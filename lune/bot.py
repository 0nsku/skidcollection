...
token="MTE4ODIxMDIwNDk5NTc2NDMwNA.GHdPoO.Hf_96mvuAH3muuTYIr1S3xT7Gu3gGP1WlqmttI"
#temp=""
def generate_key():
...
class NeoContext(commands.Context): 
 ...
 async def cmdhelp(self): 
    ...
    embed = discord.Embed(color=bot.color, title="Command: " + commandname.capitalize(), description=command.description)
    ...
    embed.add_field(name="Usage", value=f"```{commandname} {command.usage if command.usage else 'This module hasnt been created yet'}```", inline=False)
 ...
 async def create_pages(self): 
  ...
  for command in self.command.commands: 
    commandname = f"{str(command.parent)} {command.name}" if str(command.parent) != "None" else command.name
    i+=1 
    embeds.append(discord.Embed(color=bot.color, title=f"Group Command: {commandname}".capitalize(), description=command.description).set_author(name=bot.user.name, icon_url=bot.user.display_avatar.url).add_field(name="Usage", value=f"```{commandname} {command.usage if command.usage else ''}```", inline=False).set_footer(text=f"Page {i}/{len(self.command.commands)}"))
...
class HelpCommand(commands.HelpCommand):
  ...
  async def send_bot_help(self, mapping):
    ...
    return await self.context.reply("**info** @ <https://lunebot.wtf>")
  ...
  async def send_command_help(self, command: commands.Command): 
    ...
    await channel.send("**info** @ <https://lunebot.wtf>")
...
class CommandClient(commands.AutoShardedBot):
    def __init__(self):
        super().__init__( shard_count=1,
            ...
            activity=CustomActivity(name="lunebot.wtf", state="lunebot.wtf"), 
            owner_ids = [1057310810440994967, 1111622383271419904]
        )
    ...
    async def create_db_pool(self):
        self.db = await asyncpg.create_pool(port="5432", database="lune", user="joy", host="localhost", password="joypassword1")
    ...
    async def on_message(self, message: discord.Message): 
      ...
      if message.content == "<@{}>".format(self.user.id): return await message.reply(embed = discord.Embed(color=self.color, description=f"{'Prefix is' if len(await self.prefixes(message)) == 1 else 'prefixes are'}: {prefixes}"))
    ...
    async def on_command_error(self, ctx: commands.Context, error: commands.CommandError):
      if isinstance(error, commands.CommandNotFound): return 
        ...
        if isinstance(error, commands.MissingPermissions): return await ctx.send_warning(f"You're **missing** the `{error.missing_permissions[0]}` permission")
      elif isinstance(error, commands.CommandOnCooldown):
        if ctx.command.name != "hit": return await ctx.reply(embed=discord.Embed(color=0xffc0cf, description=f"{ctx.author.mention}: You are on **cooldown,** please try again in {format_timespan(error.retry_after)}"), mention_author=False)    
       ...
       await self.ext.send_error(ctx, f"You've encountered an error, please join our [**support server**](https://discord.gg/EyH4E8WFZg) and provide us with the code: `{key}`")   
...
async def on_guild_join(guild):
    channel_id = 1190408935694422036 
    ...
    try:
        ...
        if human_members < 20:
            ...
            first_channel = discord.utils.get(guild.text_channels, position=0)  
            embed = discord.Embed(
                description=f"Thank you **so much** for your interest in **lune**.\n\nSadly, your server needs **20+ __human__ members** to be able to use the bot!\nPlease [re-invite](https://discord.com/api/oauth2/authorize?client_id=1177996413087514715&permissions=8&scope=bot%20applications.commands) once you pass this!",
                color=bot.color
            )
            embed.set_footer(text="I will be leaving the server now, sorry for the inconvenience")
            await first_channel.send(owner, embed=embed)
            await guild.leave()
        else:
            embed = discord.Embed(
                color=bot.color,  
                description=f"Joined **`{guild.name}`** owned by **`{guild.owner.name}`**, which has **`{guild.member_count}`** members",
            )
            embed.set_footer(text=f"Lune now has {sum(g.member_count for g in bot.guilds):,} total users, and {len(bot.guilds):,} servers")
            await channel.send(embed=embed)
    except Exception as e:
        info_logger.info(f"Error sending message: {e}, but bot joined server")
...
async def on_guild_remove(guild):
    ...
    try:
        ...
        if human_members < 20:
            description = f"Left `{guild.name}` owned by **`{guild.owner.name}`**, which had **less than 20 human members**."
        else:
            description = f"Left `{guild.name}` owned by **`{guild.owner.name}`**, which had **`{guild.member_count}`** members."
        ...
        error_logger.error(f"Error sending message: {e}, but bot left server")
