...
class Chat(commands.Cog): 
  ...
  async def on_autoresponder(self, message: discord.Message): 
    if message.author.bot:
        return
    triggers = await self.bot.db.fetch("SELECT trigger, response FROM autoresponder WHERE guild_id = $1", message.guild.id)
    
    for trigger_response in triggers:
        trigger = trigger_response['trigger'].lower()
        ...
        if trigger in message.content.lower():
            ...
            if '{delete}' in trigger_response['response']: 
                await message.delete()
                reply = trigger_response['response'].replace("{delete}", "")
            else:
                reply = trigger_response['response']
            ...
            try: 
                ...
                await message.channel.send(EmbedBuilder.embed_replacement(message.author, reply))
            break
  ...
  async def on_autoreact(self, message: discord.Message): 
    ...
    triggers = await self.bot.db.fetch("SELECT trigger, emojis FROM autoreact WHERE guild_id = $1", message.guild.id)
    
    for trigger_row in triggers:
        trigger = trigger_row['trigger'].lower()
        ...
        if trigger in message.content.lower():
            ...
            emojis = json.loads(trigger_row['emojis'])
            for emoji in emojis: 
                try: 
