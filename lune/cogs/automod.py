...
class AutoMod(commands.Cog): 
      ...
      if not res1:  
          ...
          if not res2: 
           ...
           if len(self.antispam_cache[str(message.channel.id)][str(message.author.id)]) > check["count"]: 
            ...
            if punishment == "delete": return await message.channel.delete_messages(messages, reason="Lune AutoModeration: Spamming Messages")
            await message.channel.delete_messages(messages, reason="Lune AutoModeration: Spamming Messages")
            if not message.author.is_timed_out(): 
              await message.channel.send(embed=discord.Embed(color=self.bot.color, description=f"{self.bot.warning} {message.author.mention}: Muted for **1 minute**, for **spamming.**"))     
              await message.author.timeout(discord.utils.utcnow() + datetime.timedelta(minutes=1), reason="Lune AutoModeration: Spamming Messages")
     ...
     if any(invite in message.content for invite in invites):
        ...
        if check is not None: 
          ...
          try:
           ...
           await message.author.timeout(discord.utils.utcnow() + datetime.timedelta(minutes=1), reason="Lune AutoModeration: Sending invites")
           await message.channel.send(embed=discord.Embed(color=self.bot.color, description=f"{self.bot.warning} {message.author.mention}: Muted for **1 minute**, for **sending invites**"))
     ...
     if any(invite in message.content for invite in invites):
        ...
        if check is not None: 
          ...
          try:
           if int(data["guild"]["id"]) == message.guild.id: return
           await message.delete()
           await message.author.timeout(discord.utils.utcnow() + datetime.timedelta(minutes=5), reason="Lune AutoModeration: Sending Invites")
           await message.channel.send(embed=discord.Embed(color=self.bot.color, title="Lune AutoModeration", description=f"{self.bot.warning} {message.author.mention}: Muted for **5 minutes**, for **sending invites**"))
          except KeyError: pass  
    
    @commands.hybrid_group(aliases=["f"], invoke_without_command=True)
    async def filter(self, ctx): 
      await ctx.create_pages()

    @filter.command(name="add", description="Filter a word", help="automod", brief="manage guild", usage="[word]")
    @Perms.get_perms("manage_guild")  
    async def f_add(self, ctx: commands.Context, *, word: str): 
     check = await self.bot.db.fetchrow("SELECT * FROM chatfilter WHERE guild_id = $1 AND word = $2", ctx.guild.id, word.lower())
     if check: return await ctx.send_warning("This word is **already** filtered") 
     await self.bot.db.execute("INSERT INTO chatfilter VALUES ($1,$2)", ctx.message.guild.id, word.lower())
     await ctx.send_success(f"**{word}** is now **filtered**")
    
    @filter.command(name="remove", description="Remove a word from filters", help="automod", brief="manage guild", usage="[word]")
    @Perms.get_perms("manage_guild")
    async def f_remove(self, ctx: commands.Context, *, word: str): 
     check = await self.bot.db.fetchrow("SELECT * FROM chatfilter WHERE guild_id = $1 AND word = $2", ctx.guild.id, word.lower())
     if not check: return await ctx.send_warning("They is **no existing** word **found**") 
     await self.bot.db.execute("DELETE FROM chatfilter WHERE guild_id = $1 AND word = $2", ctx.message.guild.id, word.lower())
     await ctx.send_success(f"Removed **{word}** from filters") 
    
    @filter.command(name="list", description="Shows all blacklisted words", help="automod")
    async def f_list(self, ctx: commands.Context):
     results = await self.bot.db.fetch("SELECT * FROM chatfilter WHERE guild_id = $1", ctx.guild.id)
     if len(results) == 0: return await ctx.send_warning("They are **no blacklisted** words found")
     i=0
     k=1
     l=0
     mes = ""
    ...
    @filter.group(name="whitelist", description="Manage whitelists for filter", help="automod", aliases=["wl"])
    async def f_whitelist(self, ctx: commands.Context): 
      await ctx.create_pages()

    @f_whitelist.command(brief="manage guild", description="Whitelist channels to filters", help="automod", name="channel")
    @Perms.get_perms("manage_guild")  
    async def f_channel(self, ctx: commands.Context, *, channel: discord.TextChannel):
     await Whitelist.whitelist_things(ctx, "chatfilter", channel)    

    @f_whitelist.command(brief="manage guild", description="Whitelist someone to filters", help="automod", name="user")
    @Perms.get_perms("manage_guild")  
    async def f_user(self, ctx: commands.Context, *, member: discord.Member): 
      await Whitelist.whitelist_things(ctx, "chatfilter", member)
    
    @filter.group(name="unwhitelist", description="Remove channels & users from filters", help="automod", aliases=["uwl"])
    async def f_unwhitelist(self, ctx): 
      await ctx.create_pages()
    
    @f_unwhitelist.command(help="automod", name="channel", description="Unwhitelist a channel from filters", brief="manage guild")
    @Perms.get_perms("manage_guild")
    async def f_unwhitelist_channel(self, ctx: commands.Context, *, channel: discord.TextChannel):
     await Whitelist.unwhitelist_things(ctx, "chatfilter", channel)
    
    @f_unwhitelist.command(help="automod", name="user", description="Unwhitelist someone from filters", brief="manage guild")
    @Perms.get_perms("manage_guild")
    async def f_unwhitelist_user(self, ctx: commands.Context, *, member: discord.Member):
     await Whitelist.unwhitelist_things(ctx, "chatfilter", member)     
    
    @filter.group(name="whitelisted", description="Shows all whitelisted users & channels", help="automod")
    async def f_whitelisted(self, ctx: commands.Context): 
      await ctx.create_pages()

    @f_whitelisted.command(name="channels", help="automod", description="Shows all whitelisted channels")
    async def f_whitelisted_channels(self, ctx: commands.Context): 
     await Whitelist.whitelisted_things(ctx, "chatfilter", "channel") 

    @f_whitelisted.command(name="users", description="Shows all whitelisted users", help="automod")
    ...
    @anti_spam.command(name="enable", description="Enable AntiSpam", aliases=['e'], help="automod", brief="manage guild")
    @Perms.get_perms("manage_guild")
    async def anti_spam_enable(self, ctx: commands.Context):       
        ...
        if check: return await ctx.send_error("**AntiSpam** is **already** enabled")
    ...
    @anti_spam.command(name="disable", description="Disable AntiSpam", help="automod", brief="manage guild")
    @Perms.get_perms("manage_guild")
    async def anti_spam_disable(self, ctx: commands.Context):
      check = await self.bot.db.fetchrow("SELECT * FROM antispam WHERE guild_id = {}".format(ctx.guild.id))        
      if not check: return await ctx.send_error("**AntiSpam** is **not** currently **enabled**")
      await self.bot.db.execute("DELETE FROM antispam WHERE guild_id = $1", ctx.guild.id)
      return await ctx.send_success("**AntiSpam** is now **disabled**") 
    ...
    async def anti_spam_punishment(self, ctx: commands.Context, punishment: str): 
     ...
     return await ctx.send_success(f"**AntiSpam** punishment has been set to **{punishment}**") 
    
    @anti_spam.command(name="seconds", description="Set AntiSpam delay", help="automod", brief="manage guild", usage="[seconds]")
    @Perms.get_perms("manage_guild")
    async def anti_spam_seconds(self, ctx: commands.Context, second: int): 
     check = await self.bot.db.fetchrow("SELECT * FROM antispam WHERE guild_id = {}".format(ctx.guild.id))        
     if not check: return await ctx.send_error("**AntiSpam** is **not** currently **enabled**") 
     if second < 1: return await ctx.send_warning(f"**AntiSpam** delay **cannot** be **less than** `1 second`") 
     await self.bot.db.execute("UPDATE antispam SET seconds = $1 WHERE guild_id = $2", second, ctx.guild.id)
     return await ctx.send_success(f"**AntiSpam** delay set to **{second}**")  
    
    @anti_spam.command(name="limit", description="Set the limit for AntiSpam", help="automod", brief="manage guild", usage="[limit]")
    @Perms.get_perms("manage_guild")
    async def anti_spam_limit(self, ctx: commands.Context, second: int): 
     check = await self.bot.db.fetchrow("SELECT * FROM antispam WHERE guild_id = {}".format(ctx.guild.id))        
     if not check: return await ctx.send_error("**AntiSpam** is **not** currently **enabled**") 
     if second < 1: return await ctx.send_warning(f"**AntiSpam** limit **cannot** be **less than** `1 second`") 
     await self.bot.db.execute("UPDATE antispam SET count = $1 WHERE guild_id = $2", second, ctx.guild.id)
     return await ctx.send_success(f"**AntiSpam** limit set to **{second}**")  
    
    @anti_spam.group(name="whitelist", description="Manage whitelists for AntiInvite", help="automod", aliases=["wl"])
    async def antispam_whitelist(self, ctx: commands.Context): 
      await ctx.create_pages()

    @antispam_whitelist.command(brief="manage guild", description="Whitelist a channel from AntiSpam", help="automod", name="channel")
    @Perms.get_perms("manage_guild")  
    ...
    @antispam_whitelist.command(brief="manage guild", description="Whitelist an user from AntiSpam", help="automod", name="user")
    @Perms.get_perms("manage_guild")  
    ...
    @anti_spam.group(name="unwhitelist", description="Unwhitelist channels or users from AntiSpam", help="automod", aliases=["uwl"])
    async def antispam_unwhitelist(self, ctx): 
      await ctx.create_pages()
    
    @antispam_unwhitelist.command(help="automod", name="channel", description="Unwhitelist a channel from AntiSpam", brief="manage guild")
    @Perms.get_perms("manage_guild")
    ...
    @antispam_unwhitelist.command(help="automod", name="user", description="Unwhitelist a user from AntiSpam", brief="manage guild")
    @Perms.get_perms("manage_guild")
    async def as_unwhitelist_user(self, ctx: commands.Context, *, channel: discord.Member):
     await Whitelist.unwhitelist_things(ctx, "antispam", channel)     
    
    @anti_spam.group(name="whitelisted", description="Show all whitelisted users & channels", help="automod")
    async def antispam_whitelisted(self, ctx: commands.Context): 
      await ctx.create_pages()

    @antispam_whitelisted.command(name="channels", help="automod", description="Show all whitelisted channels")
    async def as_whitelisted_channels(self, ctx: commands.Context): 
     await Whitelist.whitelisted_things(ctx, "antispam", "channel") 

    @antispam_whitelisted.command(name="users", description="Show all whitelist users", help="automod")
    async def as_whitelisted_users(self, ctx: commands.Context): 
      await Whitelist.whitelisted_things(ctx, "antispam", "user")

    @commands.hybrid_group(name="anti-invite", invoke_without_command=True, aliases=["antiinvite"])
    async def anti_invite(self, ctx: commands.Context): 
     await ctx.create_pages()

    @anti_invite.command(name="enable", description="Enable AntiInvite", aliases=["e"], help="automod", brief="manage guild") 
    @Perms.get_perms("manage_guild")
    async def antiinvite_enable(self, ctx: commands.Context):      
        ...
        if check: return await ctx.send_error("**AntiInvite** is already **enabled** in this **server**")
        await self.bot.db.execute("INSERT INTO antiinvite VALUES ($1)", ctx.guild.id)
        return await ctx.send_success("**AntiInvite** is now **enabled**")
    
    @anti_invite.command(name="disable", description="Disable AntiInvite", aliases=["d"], help="automod") 
    @Perms.get_perms("manage_guild")
    async def antiinvite_disable(self, ctx: commands.Context): 
        check = await self.bot.db.fetchrow("SELECT * FROM antiinvite WHERE guild_id = {}".format(ctx.guild.id))        
        if not check: return await ctx.send_error("**AntiInvite** isnt **enabled**")
        await self.bot.db.execute("DELETE FROM antiinvite WHERE guild_id = $1", ctx.guild.id)
        return await ctx.send_success("**AntiInvite** has been **disabled**")
    
    @anti_invite.group(name="whitelist", description="Manage whitelists for AntiInvite", help="automod", aliases=["wl"])
    async def antiinvite_whitelist(self, ctx: commands.Context): 
      await ctx.create_pages()

    @antiinvite_whitelist.command(brief="manage guild", description="Whitelist a channel from AntiInvite", help="automod", name="channel")
    @Perms.get_perms("manage_guild")  
    ...
    @antiinvite_whitelist.command(brief="manage guild", description="Ahitelist an user from AntiInvite", help="automod", name="user")
    @Perms.get_perms("manage_guild")  
    ...
    @anti_invite.group(name="unwhitelist", description="Unwhitelist users or channels from AntiInvite", help="automod", aliases=["uwl"])
    ...
    @antiinvite_unwhitelist.command(help="automod", name="user", description="Unwhitelist a user from AntiInvite", brief="manage guild")
    @Perms.get_perms("manage_guild")
    async def unwhitelist_user(self, ctx: commands.Context, *, channel: discord.Member):
     await Whitelist.unwhitelist_things(ctx, "antiinvite", channel)     
    
    @anti_invite.group(name="whitelisted", description="Show all whitelisted users and channels", help="automod")
    async def antiinvite_whitelisted(self, ctx: commands.Context): 
      await ctx.create_pages()

    @antiinvite_whitelisted.command(name="channels", help="automod", description="Show all whitelisted channels")
    async def whitelisted_channels(self, ctx: commands.Context): 
     await Whitelist.whitelisted_things(ctx, "antiinvite", "channel") 

    @antiinvite_whitelisted.command(name="users", description="Show all whitelisted users", help="automod")
    async def whitelisted_users(self, ctx: commands.Context): 
