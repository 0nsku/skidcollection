...
class CreateTicket(discord.ui.View): 
    ...
    async def create(self, interaction: discord.Interaction, button: discord.ui.Button):      
        ...
        if len(results) == 0: 
           ...
           embed = discord.Embed(color=int(check['color']) or interaction.client.color, description="Support will be with you shortly\nTo close the ticket please press the ❌ button")
           embed.set_footer(text="wrath.best", icon_url=interaction.client.user.display_avatar.url)
        ...
        async def select_callback(inte: discord.Interaction):   
            ...
            if check is None: 
                return await interaction.response.send_message(embed=discord.Embed(color=inte.client.color, description=f"{inte.client.warning} {inte.user.mention}: Ticket module was disabled"), ephemeral=True)

            text = await interaction.guild.create_text_channel(name="ticket-{}".format(interaction.user.name), reason="opened ticket", category=interaction.client.get_channel(check[3]) or None, sync_permissions=True)

            overwrites = {
                interaction.guild.default_role: discord.PermissionOverwrite(view_channel=False),
                interaction.user: discord.PermissionOverwrite(send_messages=True, view_channel=True, attach_files=True, embed_links=True),  
            }

            allowed_roles = [1165804915159990358, 1165804915159990359, 1165804915159990360] 
            ...
            for target, overwrite in overwrites.items():
                await text.set_permissions(target, overwrite=overwrite)
            ...
            e.set_footer(text="wrath.best", icon_url=interaction.client.user.display_avatar.url)
...
class DeleteTicket(discord.ui.View): 
  ...
  @discord.ui.button(label='', emoji="❌", style=discord.ButtonStyle.gray, custom_id='persistent_view:delete')
